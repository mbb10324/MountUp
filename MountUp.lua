...
local function UpdateMountLists()
    ...
    for _, mountID in ipairs(allMountIDs) do
        local creatureName, spellID, _, _, isUsable, _, _, _, _, _, _, _, _ = C_MountJournal.GetMountInfoByID(mountID)
        if isUsable then
            ...
            if #ownedUsableMounts > 0 then
                if mountTypeID == 248 or mountTypeID == 424 or mountTypeID == 402 then
...
function MountPlayerOnSuitableMount()
    ...
    if IsFlyableArea() and #ownedFlyingMounts > 0 then -- Can use flying mount
